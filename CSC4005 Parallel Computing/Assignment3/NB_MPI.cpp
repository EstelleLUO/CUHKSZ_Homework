...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < particle_num; i++) {
        MPI_Bcast(&(particle[i].m), 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        MPI_Bcast(&(particle[i].vx), 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        MPI_Bcast(&(particle[i].vy), 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        MPI_Bcast(&(particle[i].x), 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        MPI_Bcast(&(particle[i].y), 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        MPI_Bcast(&(particle[i].fx), 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        MPI_Bcast(&(particle[i].fy), 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        MPI_Bcast(&(particle[i].valid), 1, MPI_INT, 0, MPI_COMM_WORLD);
    }
    ...
    while (current_time < compute_time){
        ...
        for (i = 0; i <particle_num; i++) {
            if (particle[i].valid == 1) {
                if (rank == i % size) Calculate_force(root, particle+i);
            }
        }

        for (i = 0; i <particle_num; i++) {
            MPI_Bcast(&(particle[i].fx), 1, MPI_DOUBLE, i % size, MPI_COMM_WORLD);
            MPI_Bcast(&(particle[i].fy), 1, MPI_DOUBLE, i % size, MPI_COMM_WORLD);
        }
        ...
        for (i = 0; i <particle_num; i++) {
            if (particle[i].valid == 1) {
                ...
                if (particle[i].x < X_L_M || particle[i].x > X_U_M) particle[i].vx = -particle[i].vx;
                if (particle[i].y < Y_L_M || particle[i].y > Y_U_M) particle[i].vy = -particle[i].vy;
