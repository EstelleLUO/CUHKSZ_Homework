...
//Store the pointer to the next position
__device__ int FAT[STORAGE_SIZE];
...
__device__ void rm(const char *fileName)
{
	...
	if (file_fcb != -1) {
		...
		for (int i = 0; i<file_system->file_num; i++) {
			...
			else if (flag == TRUE && i == file_system->file_num - 1) file_system->file_list_time[i] = 0;
		}
		flag = FALSE;
		for (int i = 0; i<file_system->file_num; i++) {
		...
		//clear file content
		int i=0;
		current_pos = FILE_STORAGE_START + file_start;
		while (i<MAX_ONE_FILE_SIZE){
			volume_d[current_pos] = 0;
			FAT[current_pos]=-2;
			if (FAT[current_pos] ==-1){
				FAT[current_pos]=-2;
				break;
			}
			current_pos++;
...
__device__ u32 write(const uchar *input, u32 size, u32 fp)
{
	...
	while (i<count){
		if (current_pos > STORAGE_SIZE-1) current_pos = 0;
		//The position is free
		if (i == 0){
			volume_d[current_pos] = input[i];
			current_pos++;
			i++
		}
		else {
			if (FAT[current_pos] ==-2){
				//Write an input
				volume_d[current_pos] = input[i];
				FAT[previous_pos] = current_pos;
				if (i == count-1) FAT[current_pos] =-1;
				previous_pos = current_pos;
				i++;
			}
			else current_pos++;
...
__device__ u32 read(uchar *output, u32 size, u32 fp)
{
	...
	if (size>fcb_table[fp].file_size) printf("Cannot read more than file size\n");
	//read the file
	int i=0;
	current_pos = FILE_STORAGE_START + file_start;
	while (i<count){
		output[i] = volume_d[current_pos];
		current_pos = FAT[current_pos];
		i++;
	}
...
__device__ void init_volume()
{
	...
	for (int i=0;i<STORAGE_SIZE;i++) FAT[i]=-2;
	for (int i = 0; i<MAX_FILE_NUM / BITS_IN_BYTE; i++) file_system->bitmap[i] = 0;
